...
# Custom JSONEncoder that handles the serialization of datetime objects
class CustomJSONEncoder(json.JSONEncoder):
    # Overriding the default method
    def default(self, obj):
        if isinstance(obj, datetime):
            # If it's a datetime object, return its ISO format string
            return obj.isoformat()
        elif isinstance(obj, date):
            # If it's a date object, also return its ISO format string
            return obj.isoformat()
        elif isinstance(obj, time):
            # If it's a time object, return it in 'HH:MM:SS' format
            return obj.strftime('%H:%M:%S')
        # Otherwise, use the default behavior (will handle non-datetime objects)
...
def save_user_profiles():
    ...
    with open('user_profiles.json', 'w') as f:
        # Dump the users data from the session state into the file using our custom encoder
        json.dump(st.session_state['users'], f, cls=CustomJSONEncoder)

# Function to load user profiles from a JSON file
def load_user_profiles():
    # Check if the user profiles file exists
    if os.path.isfile('user_profiles.json'):
        # Open the file in read mode
        with open('user_profiles.json', 'r') as f:
            try:
                # Try to load the JSON data and return it
                return json.load(f)
            except json.JSONDecodeError:
                # If JSON decoding fails, show an error and start with an empty list
                st.error("JSON decoding failed. Starting with an empty profile list.")
                return {}
    else:
